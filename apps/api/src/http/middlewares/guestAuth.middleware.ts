...
import { getOrCreateGuestId } from "@/http/auth/lib/guestId.js"
...
export const guestAuthMiddleware = () => {
  return async (c: Context, next: Next) => {
    // Always get or create guest ID
    const guestId = getOrCreateGuestId(c)
    ...
    if (sessionToken) {
      try {
        ...
        Logger.debug("Session validation failed, continuing as guest", { error })
      }
    }
