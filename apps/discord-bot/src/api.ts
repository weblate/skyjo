...
export class ApiClient {
  ...
  async kickPlayer(gameCode: string, playerId: string): Promise<void> {
    try {
      const response = await fetch(`${this.baseUrl}/api/game/${gameCode}/kick`, {
        method: 'POST',
        headers: {
  ...
  async checkGameStatus(gameCode: string): Promise<{ isActive: boolean; hasPlayer: boolean }> {
    try {
      const response = await fetch(`${this.baseUrl}/api/game/${gameCode}/status`)
      
      if (!response.ok) {
        throw new Error(`Failed to check game status: ${response.statusText}`)
      }

      const data = await response.json() as { isActive?: boolean; hasPlayer?: boolean }
      return {
